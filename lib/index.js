...
class ISYPlatform {
	...
	createAccessory(device) {
		...
		} else if (device instanceof _isydevice.ISYLeakDevice) {
			return new ISYLeakSensorAccessory(that.logger.bind(that), device);
...
class ISYThermostatAccessory extends ISYBaseAccessory {
	...
	getCurrentTemperature(callback) {
		this.log(`Getting Current Temperature - Device says: ${this.device.getCurrentTemperatureState()} translation says: ${this.toCelsius(this.device.getCurrentTemperatureState())}`);
	...
	getTargetTemperature(callback) {
		this.log(`Getting Temperature - Device says: ${this.device.getCurrentTemperatureState()} translation says: ${this.toCelsius(this.device.getCurrentTemperatureState())}`);
...
class ISYLeakSensorAccessory extends ISYBaseAccessory {
	constructor(log, device) {
		super(log, device);
	...
	getServices() {
		...
		sensorService.getCharacteristic(Characteristic.LeakDetected).on("get", this.getCurrentLeakState.bind(this));
